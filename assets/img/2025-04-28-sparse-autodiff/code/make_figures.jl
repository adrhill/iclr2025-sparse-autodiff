...
color_transparent = convert(HSL, RGBA(0, 0, 0, 0))
...
function draw!(O::DrawOperator, center)
    ...
    fontsize(O.fontsize)
    return text(
        O.text, center - Point(0.15 * O.cellsize, 0.0); halign=:center, valign=:middle
    )
end
...
function big_conv_jacobian()
    ...
    DJF = DrawOverlay(;
        text=L"J_{g}(x)", color=color_G, fontsize=150, width=350, height=180
    )
...
function forward_mode()
    ...
    DF1 = DrawMatrix(; mat=F, mat_text=F1_text, color=color_F, dashed=true, show_text=true)
    DF2 = DrawMatrix(; mat=F, mat_text=F2_text, color=color_F, dashed=true, show_text=true)
...
function sparse_map_colored()
    ...
    DS = DrawMatrix(;
        mat=S, color=color_F, dashed=true, show_text=true, column_colors=column_colors
    )
    PS = Position(DS, Point(0.0, 0.0))
    return draw!(PS)
end
...
function sparsity_pattern()
    ...
    column_colors = fill(color_black, 5)
...
function sparsity_coloring()
    ...
    DP = DrawMatrix(;
        mat=P, mat_text=P_text, color=color_F, show_text=true, column_colors=column_colors
    )
    PP = Position(DP, Point(0.0, 0.0))
    return draw!(PP)
end
...
function sparse_ad()
    ...
    DSv = DrawMatrix(;
        mat=S * v, color=color_F, absmax=absmax, show_text=true, column_colors=[c1]
    )
